...
class Environment(object):
    ...
    def restart(self, data_flag, init=False):
        ...
        for i in range(self.num_agents):
            self.states_all[i][0] = [self.states[starting_point][0], self.states[starting_point][1], self.soc_all[i], 0.065, 24]
    ...
    def act(self, action, steps):
        ...
        for i in range(self.num_agents):
            self.states_all[i][0] = [self.states[starting_point+steps, 0], self.states[starting_point+steps, 1], self.soc_all[i], eprice, 24 - (steps + 0) % 24]
        return reward
