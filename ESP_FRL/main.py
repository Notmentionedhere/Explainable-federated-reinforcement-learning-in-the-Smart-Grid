...
def args_init():
    ...
    mainarg.add_argument("--gpu_fraction",      type=float,     default=0.8,        help="")
    mainarg.add_argument("--epochs",            type=int,       default=100,        help="")
    ...
    mainarg.add_argument("--train_episodes",    type=int,       default=10,        help="") #original 100 #90
...
def train_multi_nets(args):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        try:
            best_epoch = 0
            for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
                agent.train(epoch, args.train_episodes, 'both')
