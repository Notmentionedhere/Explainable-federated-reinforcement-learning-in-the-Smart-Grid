...
class FRLDQN(object):
    ...
    def build_dqn(self):
        ...
        def build_stateq(name, weight, concat_q, summary=summary):
            with tf.variable_scope(name):
                #concat_q = tf.concat(agent_q, axis=1)
                hidden_layer, weight['sq_h_w'], weight['sq_h_b'] = self.linear(concat_q, 64, tf.nn.relu, name='stateq_h')
    ...
    def train(self, minibatch):
        ...
        #targets = np.zeros([3, len(actions)], dtype=np.float16)
            ...
            if self.preset_lambda:
                ...
                qvalue = self.frl_q.eval(
                    {self.agent_q_input[0]: q_all[0], self.agent_q_input[1]: q_all[1]
                     ...
                     #self.agent_q_input[4]: q_all[4]}) #(3,?,5)
