...
class FRLDQN(object):
    ...
    def build_dqn(self):
        ...
        def build_nn(name, weight, s_t, summary=summary):
            ...
            with tf.variable_scope(name):
                ...
                #l1, weight['l1_w'], weight['l1_b'] = self.linear(s_t, 256, name='l1')
        ...
        def build_mlp(name, weight, alpha_q, beta_q, summary=summary):
            ...
            with tf.variable_scope(name):
                ...
                # #                                                                  name='mlp_hidden')
                # mlp_output, weight['mlp_q_w'], weight['mlp_q_b'] = self.linear(hidden_layer, self.num_actions,
                #                                                                name='mlp_q')
        ...
        if self.args.train_mode == 'single_alpha':
            ...
            if self.preset_lambda:
                ...
                for name in self.frl_w:
                    #self.frl_t_w_assign_op[name].eval({self.frl_t_w_input[name]: self.frl_t_w[name].eval()})  #original
    ...
    def train(self, minibatch):
        ...
        if self.args.train_mode == 'single_alpha':
            ...
            if self.preset_lambda:
                ...
                a = self.lambda_ * alpha_postq + (1 - self.lambda_) * beta_postq
                ...
                post_q_alpha = self.alpha_t_q.eval({self.s_a: post_states_alpha}) #post_q_alpha: hist_len (32) , number_actions 25
    ...
    def load_weights(self, weight_dir):
        ...
        if self.args.train_mode == 'full':
            ...
            self.frl_w_input, self.frl_w_assign_op = self.update_q_network_op(self.frl_w, 'load_frl_pred_from_pkl')
            for name in self.frl_w:
                self.frl_w_assign_op[name].eval(
                    {self.frl_w_input[name]: load_pkl(os.path.join(weight_dir, 'frl_%s.pkl' % name))})
